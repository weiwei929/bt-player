...
chown -R www-data:www-data /opt/puresource-playlist/data
...
- 必须确保 `/opt/puresource-playlist/data/` 目录本身归 `www-data:www-data`，否则新版第一次写入会因无法创建 `resources.tmp` 返回 HTTP 500。
- 不同步 `.venv`。
- 不同步本机测试日志。
